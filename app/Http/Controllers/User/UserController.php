...
class UserController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        $validate = $request->validate([
            ...
            'email' => ['required', 'email', 'string', 'unique:masyarakat,email'],
            'username' => ['required', 'string', 'regex:/^\S*$/u', 'unique:masyarakat,username'],
            'password' => ['required', 'min:6'],
            'telp' => ['required', 'digits_between:9,15', 'unique:masyarakat,telp'],
        ]);
    ...
    public function storePengaduan(Request $request)
    {
        ...
        if ($request->hasFile('foto')) {
            $file_name = time() . '-' . $request->foto->getClientOriginalName();
            ...
            $pengaduan = Pengaduan::create([
                ...
                'judul_laporan' => $validate['judul_laporan'],
                'isi_laporan' => $validate['isi_laporan'],
                'tgl_kejadian' => $validate['tgl_kejadian'],
                'lokasi_kejadian' => $validate['lokasi_kejadian'],
                'kategori_kejadian' => $validate['kategori_kejadian'],
                'foto' => $file_name,
                'status' => '0',
            ]);
        }
